...
public class ObjLoader {
	@FunctionalInterface
	public static interface MeshCreator<T extends Mesh> {
		T create(float[] posArray, int[] indexArray, float[] coordArray);
	}
	
	public static MeshCreator<InstancedMesh> toInstancedMesh(int instances) {
		return (pos, index, coord) -> new InstancedMesh(pos, index, coord, instances);
	}
	public static MeshCreator<Mesh> toMesh() {
		return (pos, index, coord) -> new Mesh(pos, index, coord);
	}
	
	public static Mesh loadMesh(String file) throws Exception {
		return loadMesh(file, toMesh());
	}
	
	public static <T extends Mesh> T loadMesh(String file, MeshCreator<T> meshCreator) throws Exception {
		...
		return reorderLists(vertices, textures, faces, meshCreator);
	}
	
	private static <T extends Mesh> T reorderLists(
		List<Vector3f> vertexList,
		List<Vector2f> coordList,
		List<Face> faceList,
		MeshCreator<T> meshCreator
	) {
		...
		return meshCreator.create(posArray, indexArray, coordArray);
	}
